...
void BllRealTimeTrans::handleRequestHorse(QByteArray result, int descriptor)
{
	...
	//返回大小
	qint32 size = 12 * sizeof(TagHorseInfo);//初始化默认模拟数据
...
* @brief 识别端发送提交实时数据指令
*/
void BllRealTimeTrans::submitRealData(DataOutput ouputStruct)
{
	//提交实时WIN数据
	submitWINOrPLA(ouputStruct, "WIN");
	submitWINOrPLA(ouputStruct, "PLA");
	submitQINOrQPL(ouputStruct, "QIN");
	submitQINOrQPL(ouputStruct, "QPL");
}
/**
...
void BllRealTimeTrans::submitWINOrPLA(DataOutput& ouputStruct,QString type)
{
	TagProtocolMsg msg;
	msg.MsgID = 8;
	msg.nDataSize = HORSENUMBER_1*sizeof(TagWPDataInfo);// N个WPDATAINFO或QDATAINFO结构数据，按马编号
	if (type == "WIN")
	...
	if (success)
	{
		...
		emit statuChanged(QString("识别端：错误，提交实时数据指令失败-%1").arg(type)); 
 }
...
void BllRealTimeTrans::submitQINOrQPL(DataOutput& ouputStruct, QString type)
{
	TagProtocolMsg msg;
	msg.MsgID = 8;
	msg.nDataSize = (HORSENUMBER_1 - 1)*HORSENUMBER_1   * sizeof(TagQDataInfo);// N个WPDATAINFO或QDATAINFO结构数据，按马编号
	if (type == "QIN")
